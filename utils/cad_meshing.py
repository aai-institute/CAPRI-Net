...
def create_cad_mesh(generator, primitives, connection_t, filename, file_type='off', p_dim=1024, \
    ...
    parts_save_dir = "csg_parts"
    if not os.path.isdir(parts_save_dir):
        os.mkdir(parts_save_dir)
    else:
    ...
    for i in range(c_dim):
        if unused_convex[i]==0:
            part_idx += 1
            part_file = os.path.join(parts_save_dir, str(part_idx) + ".csv")
            part_primitives = []
            ...
            for j in range(p_dim):
                if connection_t[0,j,i]>0:
                    ...
                    primitive_parameters = original_primitives[:,:,j:j+1]
                    part_primitives.append(primitive_parameters)
            ...
            np.savetxt(part_file, np.array(part_primitives).reshape(-1, 7), delimiter=",")
